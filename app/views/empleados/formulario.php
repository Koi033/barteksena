<?php
/* app/views/empleados/formulario.php - Formulario crear/editar empleado */

// En modo edición, el registro solo trae "nombre_completo" (una sola columna
// en la BD). Lo separamos en nombre/apellido únicamente para prellenar
// los dos campos del formulario; al guardar se vuelven a unir.
$nombrePre   = '';
$apellidoPre = '';
if ($empleado) {
    $partes      = explode(' ', trim($empleado['nombre_completo']), 2);
    $nombrePre   = $partes[0] ?? '';
    $apellidoPre = $partes[1] ?? '';
}
?>
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" id="fNombre" name="nombre"
                    ...
                    value="<?= htmlspecialchars($nombrePre, ENT_QUOTES, 'UTF-8') ?>">
            </div>

            <div class="form-group">
                <label for="fApellido">Apellido *</label>
                <input type="text" id="fApellido" name="apellido"
                    maxlength="75" required
                    value="<?= htmlspecialchars($apellidoPre, ENT_QUOTES, 'UTF-8') ?>">
            </div>
